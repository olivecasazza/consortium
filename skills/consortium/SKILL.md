...
The node name is substituted for `%h` before the shell sees the line, and the
command is single-quoted unless it contains only characters that need no quoting.
With `claw -w node1 uptime` and no other flags, that expands to:

```
ssh node1 uptime
```

Two documented injection points control the transport. Both are ordinary flags,
not environment variables, and neither requires credentials of any kind on the
command line.

### Injection point 1: `--remote` swaps the transport binary
...
```bash
#!/bin/sh
# /usr/local/bin/cluster-ssh — stand-in for ssh, forces the jump host
...
### Injection point 2: `-o` / `--options` passes ssh options verbatim
...
When the value begins with a hyphen, a space-separated assignment breaks. clap
reads the next token as another set of short flags, not as the value of
`--options`. `claw --options -F/etc/ssh/cluster_config` therefore does not pass
the config file; it sets claw's own `-F` output format and leaves the ssh config
unused.

Always attach the value with `=` and let the shell quote it:

```bash
claw -w 'node[1-4]' --options=-F/etc/ssh/cluster_config hostname
```

The same rule applies to any other value starting with `-`, including a
`ProxyCommand` value that names a flag-like path.
...
For `node1`, claw assembles the line, then substitutes the node name for every
`%h`:

```
ssh -o ProxyCommand="ssh -W %h:%p jump-host.example.com" -o ConnectTimeout=10 node1 'systemctl is-system-running'
```
...
ssh then opens a connection to `jump-host.example.com` and asks it to forward
stdin and stdout to `node1` port 22. The jump host never needs a claw group of
its own, and the target nodes never need to resolve each other.

The same pattern belongs in `~/.ssh/config` when the jump host is permanent:
...
Point claw at a non-default config file with the equals-sign form,
`--options=-F/path/to/config`.

## Groups and groups.conf

`-g`, `-X`, `-a`, and every `pinch` group query read `groups.conf`. claw looks in
two places, in this order:

1. `~/.config/clustershell/groups.conf`
2. `/etc/clustershell/groups.conf`

Both files are loaded when both exist. Every section other than `[Main]`
becomes a group source, and each one needs a `map` or `mapall` upcall or the
load fails. A section name defined in both files resolves from the file read
last, which is the system one. When neither file exists, any group-based
