...
fn test_026_index() {
    ...
    check(
        &["--index", "bmc10", "node[1-4],bmc[10-20]"],
        None,
        Some("0\n"),
        0,
        None,
    ...
    check(
        &["--index", "node1", "node[1-4],bmc[10-20]"],
        None,
        Some("11\n"),
        0,
        None,
    ...
    // nD nodeset
    check(
        &["--index", "da34p1", "da[30,34-51]p[1-2]"],
        None,
        Some("2\n"),
        0,
        None,
    ...
    check(
        &["--index", "prune3", "prune[003-034]"],
        None,
        Some(""),
        1,
        Some("ERROR: 'prune3' is not in nodeset\n"),
    );
    ...
    check(
        &["--index", "node7", "-I", "0-4", "node[0-9]"],
        None,
        Some(""),
        1,
        Some("ERROR: 'node7' is not in nodeset\n"),
    );
    // missing node: ValueError -> exit 1 (like list.index())
    check(
        &["--index", "node42", "node[0-9]"],
        None,
        Some(""),
        1,
        Some("ERROR: 'node42' is not in nodeset\n"),
    );
    // a single node is required
    check(
        &["--index", "node[1-2]", "node[0-9]"],
        None,
        Some(""),
        1,
        Some("ERROR: index() argument must be a single node\n"),
    );
    // --index is a command: incompatible with other commands and --pick
    check(
        &["--index", "node5", "-c", "node[0-9]"],
        None,
        None,
        2,
        Some("Multiple commands not allowed.\n"),
    );
    check(
        &["--index", "node5", "--pick", "2", "node[0-9]"],
        None,
        None,
        2,
        Some("--index cannot be combined with --pick\n"),
    );
}
...
fn test_027_index_rangeset() {
    ...
    check(
        ...
        Some(""),
        1,
        Some("ERROR: 50 is not in RangeSet\n"),
    );
}
