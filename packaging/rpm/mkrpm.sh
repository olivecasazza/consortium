...
if [ $# -lt 1 ]; then
    ...
    echo "Build sdist tarball and RPMS from packaging/rpm/clustershell.spec.in"
    echo "example: $0 el7"
    exit 1
fi
...
sed -e "s/^Version:\(\s\+\)%{version}$/Version:\1${VERS}/" <packaging/rpm/clustershell.spec.in >packaging/rpm/clustershell.spec

rpmbuild --define "dist .$DIST" --define "_sourcedir $PWD/dist" -ba packaging/rpm/clustershell.spec
